...
[ -n "${SRC_TOKEN}" && "${TOKEN}" =~ "%TOKEN%" ] && AUTH_SRC=${AUTH_SRC/%TOKEN%/${SRC_TOKEN}/} && echo "$SRC: %TOKEN is set"
...
[ -n "${DEST_TOKEN}" && "${TOKEN}" =~ "%TOKEN%" ] && AUTH_DEST=${AUTH_DEST/%TOKEN%/${DEST_TOKEN}/} && echo "$DEST: %USER is set"
